...
public class GazePointDataComponent : MonoBehaviour
{
    ...
    protected void Update()
    {
		...
		pos = Camera.main.ScreenToWorldPoint (new Vector3 (LastGazePoint.Screen.x, LastGazePoint.Screen.y, 1f));
		transform.position = Vector3.Lerp (transform.position, pos, Time.deltaTime  * 3f);

		Collider2D c = Physics2D.OverlapCircle (pos, 1f);
		if (c != null) {
			c.SendMessage ("OnEyeHover", SendMessageOptions.DontRequireReceiver);
		}
    }
}
